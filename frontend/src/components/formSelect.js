  ...
  return (
    <div className={`flex flex-col gap-1 mt-4 max-md:mt-3 ${className}`}>
      ...
      <select
        id={id}
        {...register}
        className="border !bg-[rgba(250,250,250,0.56)] border-gray-300 rounded-md px-3 py-2 text-base max-md:text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
      >
        {options.map((opt) => (
